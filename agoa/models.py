...
class Turnaround(models.Model):
    ...
    arrival_flight = models.OneToOneField(Flight, related_name='arrival_turnaround', on_delete=models.CASCADE)
    departure_flight = models.OneToOneField(Flight, related_name='departure_turnaround', on_delete=models.CASCADE)
    ...
    def clean(self):
        ...
        if self.arrival_flight.arrival_airport != self.airport or self.departure_flight.departure_airport != self.airport:
            raise ValidationError("Les vols doivent arriver et partir du même aéroport que le turnaround")
